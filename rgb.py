...
class PCV:
    ...
    def proses_gambar(folder_path):
        data = {'Nama Gambar': [], 'Avg_R': [], 'Avg_G': [], 'Avg_B': []}
        ...
        for namagambar in os.listdir(folder_path):
            ...
            if path_file.lower().endswith(('.png', '.jpg', '.jpeg')):
                output_path = os.path.join("hasil_removeBG", f"hasil_{namagambar.split('.')[0]}.png")
                PCV.remove_background(path_file, output_path)
                ...
                if gambar_asli is not None:
                    rgb_real = PCV.ambil_rgb_real(gambar_asli)
                    # label = SistemCerdasPCV.label_kematangan(rgb_real)

                    data['Nama Gambar'].append(namagambar)
                    data['Avg_R'].append(rgb_real[0])
                    data['Avg_G'].append(rgb_real[1])
                    data['Avg_B'].append(rgb_real[2])
...
if __name__ == "__main__":
    folder_path = "D:/New folder/baru/Tomat Mentah"
    # os.makedirs("hasil_cropping_Setengah Matang", exist_ok=True)
    PCV.proses_gambar(folder_path)
